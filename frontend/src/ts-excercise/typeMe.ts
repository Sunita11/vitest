...
function getEventById(id: EventId): EventItems | null {
  const idStr = "" + id;
  return events.filter((e) => "" + e.id === idStr)[0];
}

// Should return an object with dateString & isPast
function getEventDate(event: EventItems): {
  dateString: string;
  isPast: boolean;
} {
...
const getEventRsvpCount = (event: EventItems) => {
...
const getEventDetails = (eventId: EventId) => {
